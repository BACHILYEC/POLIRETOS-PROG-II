...
public class MatrizIniciales {
    ...
    private static boolean dibujarLetra(char letra, int fila, int col, int n) {
        ...
        return switch (letra) {
            ...
            case 'B' -> col == 0 || fila == 0 || fila == mid || fila == n - 1 || (col == n - 1 && (fila < mid || fila > mid));
            ...
            case 'G' -> fila == 0 || fila == n - 1 || col == 0 || (col == n - 1 && fila >= mid) || (fila == mid && col >= mid);
            ...
            case 'M' -> col == 0 || col == n - 1 || (fila == col && fila <= mid) || (fila + col == n - 1 && fila <= mid);
            case 'N' -> col == 0 || col == n - 1 || fila == col;
            case 'O' -> fila == 0 || fila == n - 1 || col == 0 || col == n - 1;
            case 'P' -> col == 0 || fila == 0 || fila == mid || (col == n - 1 && fila < mid);
            case 'Q' -> fila == 0 || fila == n - 1 || col == 0 || col == n - 1 || (fila == col && fila >= mid);
            case 'R' -> col == 0 || fila == 0 || fila == mid || (col == n - 1 && fila < mid) || fila - col == mid;
            case 'S' -> fila == 0 || fila == mid || fila == n - 1 || (col == 0 && fila < mid) || (col == n - 1 && fila > mid);
            case 'T' -> fila == 0 || col == mid;
            case 'U' -> col == 0 || col == n - 1 || fila == n - 1;
            case 'V' -> (col == fila && fila >= mid) || (col + fila == n - 1 && fila >= mid);
            case 'W' -> col == 0 || col == n - 1 || (fila + col == n - 1 && fila >= mid) || (fila == col && fila >= mid);
            case 'X' -> fila == col || fila + col == n - 1;
            case 'Y' -> (fila == col && fila < mid) || (fila + col == n - 1 && fila < mid) || (col == mid && fila >= mid);
